...
**Reference studied first, per the brief:** cloned and read [`Daudxu/CharacterControl-threejs`](https://github.com/Daudxu/CharacterControl-threejs) before writing any code. It's a floating-capsule WASD character controller (movement/jump/moving-platforms) -- useful for the basic `<RigidBody>`/`<CapsuleCollider>`/`useRapier()` API patterns and the "camera follows the physics body's real transform" idea, but it doesn't solve a rope-swing mechanic at all (no existing open-source repo does, in our exact stack) -- the joint-based swing itself was designed from Rapier's own joint-hook documentation (`useSphericalJoint`, "typically used to simulate ragdoll arms, pendulums, etc.").
...
- **`PhysicsSwingRig.tsx`**: a dynamic capsule-collider character. The moment `phase` becomes `"swinging"` (the same optimistic, pre-on-chain-confirmation trigger the old scripted rig used -- this part of the flow is untouched), a spherical joint (ball-and-socket) attaches between the character and the *next* anchor building; gravity plus that constraint produce the pendulum arc, not a precomputed curve. The joint is released (by unmounting a tiny `<SwingJoint>` wrapper -- react-three-rapier's `useSphericalJoint` creates on mount, removes on unmount) the instant `phase` changes away from `"swinging"`. On a successful landing, the character is snapped onto the confirmed exact anchor point -- a small correction, not a teleport, since the physics motion already carried it most of the way there. On a miss, nothing extra happens: the joint is already gone, so gravity and whatever velocity the character already had take over as a **real physics fall against/away from the buildings**, not a scripted drop curve.
